...
STATIC mp_obj_t py_tf_regression(uint n_args, const mp_obj_t *args, mp_map_t *kw_args)
{
    ...
    printf("width of input np array = %i", input_size_width);
    printf("height of input np array = %i", input_size_height);
    printf("size of output array = %i", output_size);

    // read input
    ndarray_obj_t *arg_input_array = args[1];

    printf("size of input np array = %i", arg_input_array->len);
    printf("size of input np array = %i", arg_input_array->shape);
